...
namespace Microsoft.Bot.Builder.Integration.AspNet.Core
{
    internal class SendQueue<T> : IDisposable
    {
        ...
        public void Post(ISendBlock<T> itemSource)
        {
            PostInternal(itemSource);
        }
            ...
            try
            {
                while (true)
                {
                    ...
                    while (true)
                    {
                        ...
                        // Queue item is sendblock
                        if (obj is ISendBlock<T> sendBlock)
                        {
                            // Get all items from source and invoke individually
                            while (true)
                            {
                                T item = default(T);
                                try
                                {
                                    item = await sendBlock.GetAsync().ConfigureAwait(false);
                                }
                                catch (Exception)
                                {
                        ...
                        {
                            // Queue item is single item (typical case)
                            try
                            {
                                // Invoke operation
